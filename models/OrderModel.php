...
class OrderModel
{
    ...
    public function getByUserId($user_id)
    {
        return $this->db->select(
            "SELECT o.id, o.created_at, o.total_amount, o.status, r.nome as nome_ristorante
             FROM orders o
    ...
    {
        return $this->db->select(
            "SELECT o.*, u.username as cliente_nome
             FROM orders o
    ...
    public function isOrarioOrdinabile($restaurant_id)
    {
        $ristorante = $this->db->selectOne(
            "SELECT orario_limite_ordini FROM ristoranti WHERE id = ?",
            [$restaurant_id]
        );

        $limite = $ristorante['orario_limite_ordini'] ?? '22:00:00';

        return date('H:i:s') <= $limite;
    }

    public function getOrarioLimite($restaurant_id)
    {
        $ristorante = $this->db->selectOne(
            "SELECT orario_limite_ordini FROM ristoranti WHERE id = ?",
            [$restaurant_id]
        );
        return $ristorante['orario_limite_ordini'] ?? '22:00:00';
    }
}
